...
from . import _analytic_rotation as ar
from . import _gpa_rotation as gr
    ...
    * :math:`\circ` is the element-wise product or Hadamard product.
    ...
    if method == 'orthomax':
        ...
        if algorithm == 'gpa':
            ...
            def vgQ(L=None, A=None, T=None):
                return gr.vgQ_partial_target(H, W=W, L=L, A=A, T=T)
        elif algorithm == 'gpa_der_free':

            def ff(L=None, A=None, T=None):
                return gr.ff_partial_target(H, W=W, L=L, A=A, T=T)
        else:
            raise ValueError('Algorithm %s is not possible for %s rotation' %
                             (algorithm, method))
    else:
...
class unittests(unittest.TestCase):
    ...
    def _test_template(self, method, *method_args, **algorithms):
        ...
        algorithm1 = 'gpa' if 'algorithm1' not in algorithms else algorithms[
            'algorithm1']
        algorithm2 = 'gpa_der_free' if 'algorithm1' not in algorithms else algorithms[
            'algorithm1']
    ...
    def test_methods(self):
        ...
        methods = [
            'quartimin', 'biquartimin', 'quartimax', 'biquartimax', 'varimax',
            'equamax', 'parsimax', 'parsimony', 'target', 'partial_target'
        ]
        for method in methods:
            ...
            if method == 'target':
                ...
                method_args = [self.get_H(), 'orthogonal']
                self._test_template(
                    method, *method_args, algorithm2='analytic')
            elif method == 'partial_target':
